    ...
    .controller('DebtsToPayCtrl', function(_, holderService, providerService, cuentaContableService, debtsToPayService, authService, contableService, configCuentasService,
        ...
        vm.addRetentionToDebtsDetail = function() {
            ...
            _.map(vm.retention.detailRetentions, function(detail) {
                if (detail.taxType == 'RETENCION EN LA FUENTE') {
                    ...
                    configCuentasService.getConfigCuentaByUserClientAndOptionCode(vm.usrCliId, 'ANX'+vm.retenCodeFuente).then(function (response){
                        if(response.code){
                            vm.retenFteCodeContable = response.code; vm.retenFteDescContable = 'RETENCIÓN EN FUENTE';
                            vm.retenFteNombContable = response.option.fullDescription; vm.retenFteValor = parseFloat(vm.retenTotalFuente);
                            vm.retenFteNombContableCxP = 'RET:' + ' ' + vm.generalDetailCxP; vm.retenFteValorCxP = parseFloat(vm.retenTotalFuente);

                            vm.addDetails();
                        } else {
                            vm.error = 'No existe unca Cuenta Contable asociada a ANEXO '+vm.retenCodeFuente;
                        }
                    }).catch(function(error) {
                        vm.loading = false;
                        vm.error = error.data;
                    ...
                    configCuentasService.getConfigCuentaByUserClientAndOptionCode(vm.usrCliId, 'RET'+vm.retenCodeIva).then(function (response){
                        if(response.code){
                            vm.retenIvaCodeContable = response.code; vm.retenIvaDescContable = 'RETENCIÓN EN IVA';
                            vm.retenIvaNombContable = response.option.fullDescription; vm.retenIvaValor = parseFloat(vm.retenTotalIva);
                            vm.retenIvaNombContableCxP = 'RET:' + ' ' + vm.generalDetailCxP; vm.retenIvaValorCxP = parseFloat(vm.retenTotalIva);

                            vm.addDetails();
                        } else {
                            vm.error = 'No existe unca Cuenta Contable asociada a RETENCION '+vm.retenCodeIva;
                        }
                    }).catch(function(error) {
                        vm.loading = false;
                        vm.error = error.data;
        ...
        vm.loadMedio = function() {
            ...
            if (vm.medio.medio === 'credito') {
                ...
                vm.status = 'PENDIENTE';
            } else if (vm.medio.medio === 'efectivo') {
                vm.medio.payForm = '20 - OTROS CON UTILIZACION DEL SISTEMA FINANCIERO';
                vm.medio.total = parseFloat((vm.debtsToPay.total - payed).toFixed(4));
                vm.status = 'PAGADO';
            };
        ...
        vm.addPago = function() {
            if(vm.medio.medio === 'efectivo'){
                var credito = {
                    payNumber: 1,
                    diasPlazo: 1,
                    estadoCredits: 'PAGADO',
                    fecha: new Date().getTime(),
                    documentNumber: vm.debtsToPay.billNumber,
                    valor: vm.debtsToPay.total
                };
                vm.medio.creditsDebts = [credito];
            }
        ...
        vm.disableMultipleCobro = function(){
            if(vm.typePayment === 'PAC' && (vm.noDocument === undefined || '' === vm.noDocument.trim() || vm.noDocument === 0) && vm.modePayment !== 'EFC'){
                return true;
            }
        ...
        function _asientoComprobanteEgreso() {
            ...
            vm.itema.numCheque =  vm.paymentDebtsCreated.noDocument;
