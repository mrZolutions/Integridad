...
public interface DailybookCxPRepository extends CrudRepository<DailybookCxP, UUID> {
    ...
    @Query("SELECT d FROM DailybookCxP d WHERE d.provider.id = (:id) ORDER BY d.dailycxpSeq")
    Iterable<DailybookCxP> findDailybookCxPByProviderId(@Param("id") UUID id);
    
    @Query("SELECT d FROM DailybookCxP d WHERE d.subsidiary.userClient.id = (:userClientId)")
    Iterable<DailybookCxP> findDailybookCxPByUserClientId(@Param("userClientId") UUID id);
    
    @Query("SELECT d FROM DailybookCxP d WHERE d.subsidiary.userClient.id = (:userClientId) AND d.provider.id = (:providerId) AND d.billNumber = (:bill)")
