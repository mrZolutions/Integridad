...
public interface CellarRepository extends CrudRepository<Cellar, UUID> {
    ...
    @Query("SELECT c FROM Cellar c WHERE c.provider.id = (:id) AND c.active = true AND c.statusIngreso = 'INGRESADO' ORDER BY c.cellarSeq")
    ...
    @Query("SELECT c FROM Cellar c WHERE c.subsidiary.userClient.id = (:userClientId) AND c.dateEnterCellar >= (:dateOne) AND c.dateEnterCellar <= (:dateTwo) AND c.active = true ORDER BY c.cellarSeq")
